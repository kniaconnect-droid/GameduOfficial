...
export default function SecureGamePlayer({
  ...
  // gameUrl = Blob URL berisi HTML game (bukan HTML string mentah lagi).
  ...
  const loadGame = React.useCallback(async () => {
    ...
    try {
      ...
      // PENTING: pakai Blob URL (iframe src) bukan srcDoc.
      // Iframe dengan srcDoc yang disisipkan secara dinamis punya bug lama di
      // WebKit/Chrome mobile -- kadang nggak ke-render/ke-paint sama sekali
      // sampai ada "forced repaint" (contoh: user pindah ke app lain lalu balik
      // lagi, atau ganti tab). Ini persis gejala "harus pindah layar dulu baru
      // game muncul". Blob URL + <iframe src=...> nggak kena bug ini karena
      // browser memperlakukannya seperti navigasi dokumen biasa.
  ...
  const restartGame = React.useCallback(() => {
    ...
    if (frame && frame.contentWindow && gameUrl) {
  ...
  useEffect(() => {
    loadGame();
    // Bersihin Blob URL yang lagi aktif pas komponen unmount / gameId ganti,
    // supaya nggak numpuk di memori browser.
    return () => {
      setGameUrl((prevUrl) => {
        if (prevUrl) URL.revokeObjectURL(prevUrl);
        return "";
